...
void Groceries::OptionTwo(string stringInput) // Option Two function used in main menu
{
	while (true) {
		try {
			...
			cout << "Please enter correct input." << endl; // If they enter invalid input they need to make a correction
			system("pause");
			system("CLS");
		}
		return;
...
int Groceries::Histogram(string gString, int gNum) { // Histogram function
	...
	if (dataFile.is_open()) {
		while (dataFile >> gString >> gNum) { // take values from user
			...
			groceryMap.mapDisplay(); // display map
